...
남학생은 스위치 번호가 자기가 받은 수의 배수이면, 그 스위치의 상태를 바꾼다. 즉, 스위치가 켜져 있으면 끄고,
꺼져 있으면 켠다. <그림 1>과 같은 상태에서 남학생이 3을 받았다면, 이 학생은 <그림 2>와 같이 3번, 6번 스위치의 상태를 바꾼다.
...
N = int(input())  # 스위치 갯수
switch = list(map(int, input().split()))  # 초기 스위치 상태
switch.insert(0, 0)

M = int(input())  # 학생 수
st_lst = [list(map(int, input().split())) for _ in range(M)]  # 학생 상태 [i] : 성별 [i][j] : 받은 카드
