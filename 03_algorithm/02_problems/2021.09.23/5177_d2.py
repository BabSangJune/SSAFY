...
날짜 : 2021.08.26
학습 : SWEA D2
제목 : 5177. [파이썬 S/W 문제해결 기본] 8일차 - 이진 힙
문제 :
...
for tc in range(1, T+1):
    ...
    tree = [0] + list(map(int, input().split())) #트리

    for i in range(2, len(tree)): # 2부터 시작

        while True:
            if tree[i] < tree[i // 2]: #부모가 자식보다 크면
    ...
    while True: # 더 하기
        result += tree[idx_sum // 2] #마지막 노드의 부모
        idx_sum = idx_sum // 2 #타고 올라가기

        if idx_sum <= 0: #맨 윗층에 오르면
            break
